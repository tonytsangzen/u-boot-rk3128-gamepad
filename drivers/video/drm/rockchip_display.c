...
static int display_logo(struct display_state *state)
{
	...
	printf("fb addr:%08x w:%d h:%d h:%d v:%d bpp:%d\n",  
		crtc_state->dma_addr,	logo->width, logo->height, hdisplay, vdisplay, logo->bpp);
...
static int do_rockchip_show_fb(cmd_tbl_t *cmdtp, int flag, int argc,
				char *const argv[])
{
	if (argc != 3)
		return CMD_RET_USAGE;
	
	uint32_t fb = simple_strtoul(argv[1], NULL, 16);
	uint32_t fmt = simple_strtoul(argv[2], NULL, 16);
	struct display_state *s;
...
U_BOOT_CMD(
	rockchip_show_fb, 3, 1, do_rockchip_show_fb,
	"display framebuffer",
	"	<framebuffer address> <fmt>"
);
